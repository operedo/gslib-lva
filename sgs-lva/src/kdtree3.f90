...
module kdtree2_module
  ...
  type :: kdtree2
      ...
      logical, pointer :: IS_USABLE(:) => null()
      logical, pointer :: REARRANGED_IS_USABLE(:) => null()
      ! indicates if a data point is usable or not.  This adds functionality so that
      ! a tree can be preloaded with points that will be searched for later at a time
  ...
  type :: tree_search_record
      ...
      logical, pointer :: usable(:)       ! temp pointer to IS_USABLE  !***NEW***
...
contains
  ...
  function kdtree2_create(nx,ny,nz,NODES,sort,rearrange) result (mr)
  use global
    !
    ...
    integer nx,ny,nz,NODES
    
    allocate(input_data(d_tree,NODES) )
    allocate(input_usable(nx*ny*nz))

    input_usable=is_usable
    input_data = transpose(coord_ISOMAP(:,1:d_tree))
    
    ! ..
    allocate (mr)
    mr%the_data => input_data
    mr%IS_USABLE => input_usable !***NEW***
    ! pointer assignment
...
!    end if
    ...
    if (mr%rearrange) then
       ...
       allocate(mr%REARRANGED_IS_USABLE(mr%n)) !***NEW***
       do i=1,mr%n
          mr%rearranged_data(:,i) = mr%the_data(:,mr%ind(i))
          
          mr%REARRANGED_IS_USABLE(i) = &
                 mr%IS_USABLE(mr%ind(i))    !***NEW***
       enddo
    else
       nullify(mr%rearranged_data)
       nullify(mr%REARRANGED_IS_USABLE)    !***NEW***
    endif
...
!    deallocate(sr%pqp)
  ...
  subroutine kdtree2_n_nearest_around_point(vecin,dim,tp,idxin,correltime,nn,results)
    ...
    if (sr%rearrange) then
       sr%Data => tp%rearranged_data
       sr%usable => tp%REARRANGED_IS_USABLE !***NEW***
    else
       sr%Data => tp%the_data
