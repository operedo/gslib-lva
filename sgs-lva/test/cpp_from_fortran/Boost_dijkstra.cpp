//this program takes 2 files grid.out and nodes2cal.out and calculates all distances from every node in grid.out
//to every node in nodes2cal.out.
//
//input
//grid.out - first line has #nodes,#edges, each additional line contains two nodes and their distances (node1 node2 dist)
//nodes2cal.out - first line is number of nodes to calcualted.  then each additional line has the node (in c++ node=nodeFORTRAN-1)
//
//output
//dist_cpp.out - a file with nodes*nodes2cal lines, nodes cycles first
// #include "stdafx.h"
#include <stdexcept>
#include <iostream> 
...
//     char dateStr [9];
//     char timeStr [9];
//     char *outfile2 ; //time data file
//     char *outfile3 ; //time data file
//     outfile2="time.out" ;
//     outfile3="time_dijk.out" ;
//     _strdate( dateStr1);
//     _strtime( timeStr1 );
//     ofstream outtime( outfile2, ios::out ) ;
//     ofstream dijtime( outfile3, ios::out ) ;
//     outtime <<  "start day                " << dateStr1 << "  time " << timeStr1 <<  endl ;
