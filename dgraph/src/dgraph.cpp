	...
	for (int i=0; i<nodes2cal; ++i) 
	{
		...
		for(int procId=0;procId<world.size();procId++){
			...
			if (world.rank() == procId){
				...
				sprintf(filename,"dist_cpp.out_%05d_%05d" ,i,procId);
		...
		//	for (graph_traits<graph_t>::vertices_size_type i = 0; i < sum; ++i) {
		//		outfile << get(distance,vertex(i,g)) << std::endl;
		//	//	graph_traits<graph_t>::vertices_size_type v = get(,vertex(i, g));
		//	//
		...
		//	for (int i = 0; i < sum; ++i) 
		//		outfile << i << "  " << get(distance, vertex(i, g)) <<std::endl;
		//}
