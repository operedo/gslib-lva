...
module graph_vars
    implicit none
    integer, allocatable, dimension (:) :: KF
    integer NODES
    integer*8 EDGES
    real*8 refine
...
subroutine set_graph(MDS_opt)
    ...
    real*8, dimension ( 3 ) :: ba !Vector b - a
    real*8, dimension (3,3) :: R  !Anisotropic rotation matrix
    real*8, dimension ( 3 ) :: v  !copy of ba
    ...
    !undocumented option to refine the grid to get more accurate paths
    ...
    !template for the offsets, some are redundant and can be skipped
    ...
    if(add_paths_land ==1 ) LANDNODES=xyzland*nx2*ny2*nz2  !number of paths based on the grid
    if(add_paths_data ==1 ) DATANODES=ndata*nx2*ny2*nz2    !number of paths based on the data
...
if(MDS_opt ==3) return 
    ...
    do i=1,nx2
        ...
        do ii=-kkk,kkk
            ...
            if(ix>0 .and. ix<=nx2 .and.   iy>0 .and. iy<=ny2 .and.   iz>0 .and. iz<=nz2 .and. redund_temp(ii,jj,kk)==0) then  !we are in the model
                    ...
                    else !actually have to calculate this distance, will take a long time
 ...
 !                   to write out the graph:
        ...
        end do !loops over the offsets
    end do
    end do
    end do !loops over the grid


!add the straightline paths from each landmark point to each node:
no_cpp=0
cnt=0
...
!calculate the anisotropic distance (jeff)
    ...
    do i=2,cnt !there are cnt+1 segments of the line
        ...
        if(length>0) then 
            ...
            andist=andist+template_D(ii,jj,kk,ind(1),ind(2),ind(3) ) * length
        end if
    end do
end function andist
!-------------------------------------------------------------------
