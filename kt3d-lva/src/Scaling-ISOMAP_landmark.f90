subroutine get_landmark_pts(ndmax,nd,nx,ny,nz)
    ...
    integer test,cnt,nodesfl,sys_call_out,j,i,k,ii,jj,kk,ndmax,nx,ny,nz
    real*4 LWORK(2)
    nodesfl=9698
    ...
    write(*,*) '*Done reading in distances from dist_cpp.out*'
    close(nodesfl)
...
subroutine MDS_ISOMAP(ndmax,nd,nx,ny,nz)    
    ...
    real*8 D1,vec_temp(xyzland),mds_diff,nx,ny,nz
    ...
    coord_ISOMAP=coord_ISOMAP**2 !get the squared dist
    ...
    start_time=secnds(0.0)
    call eig ( subB2, evalues, vectors, ubound(subB2,1) )  !from LAPACK library, need to include library to build this program
    write(T_eigs,'(3I12,f19.5)')  NODES, edges, ubound(subB2,1), secnds(start_time)
    Write(*,*) 'DONE EigenSolution, calculate the coord of all grid points'
    
    !need to reverse array order
    !use prexisting arrays as temp arrays
    subB2=-999 !array for eigenvectors
    A1=-999 !array for eigenvalues
    ...
    if(dim==xyzland) dim=xyzland-1 !discard the smallest if there are none neg, as per ISOMAP
    if(dim==0 .or. dim==-1) dim=xyzland-1 !discard the smallest if there are none neg, as per ISOMAP
...
!$omp end parallel
    ...
    !coord_ISOMAP NOW HAS THE COORD FOR EACH GRID LOCATION
...
!************************
    ...
    do i=1,NODES
     write(47861,'(3000f18.8)') coord_ISOMAP(i,1:3)
    end do

MDS_diff=1
stress = 0
sum_dist = 0
stress_nodes  = xyzland  !only have the original distances to the landmark points, not the full distance matrix
