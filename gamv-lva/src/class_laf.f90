...
module class_lvaf2
    ...
    !PRIVATE PARAMETERS
    real(kind=dp), private, parameter :: PI      = 3.14159265358979D+00
    real(kind=dp), private, parameter :: DEG2RAD = PI / 180.0D+00
    real(kind=dp), private, parameter :: eps = 1.0e-4
    ...
    !PRIVATE VARIABLES
    real(kind=dp), private, allocatable, dimension (:,:) :: dsl !line segmented by a grid
  ...
  contains
    ...
    !solution.
    real(kind=dp) function min_dsqrd ( a, b ) result ( D )
        real(kind=dp), dimension ( 3 ), intent (in) :: a, b !line endpoints
        real(kind=dp), allocatable, dimension (:,:) :: L1, L2 !lines
        real(kind=dp) :: Dn, D0
        integer :: i, j, k, n, test1
        
        !Determine a maximum number of line refinements to use
        n = block_count(a, b)
        if(n < 0) call gen_error ( 'line endpoint beyond grid spec', 'min_dsqrd' )
    ...
    real(kind=dp) function min_dsqrd_ ( L, n, D0 ) result ( D )
        use global
        integer, intent (in) :: n !# of vertices
        real(kind=dp), dimension (n,3), intent (inout) :: L !line
        real(kind=dp), intent (out) :: D0 !Initial distance
        !INTERNALS
        real(kind=dp) :: t, Dt !Newton step parameter and distance for that step
        integer :: i, j
        logical :: outside,err
    ...
    subroutine fnc_eval ( n, Line, F, error )
        use global
        integer,               intent (in)  :: n !# of vertices
        real(kind=dp),          intent (out) :: F !Function (distance)
        real(kind=dp), dimension (n,3), intent (in) :: Line !line
        !INTERNALS
        real(kind=dp), dimension (3,3) :: R !Rotation matrix
        real(kind=dp), dimension ( 3 ) :: angles
        real(kind=dp), dimension ( 2 ) :: ratios
        real(kind=dp) :: t, dj
        integer :: i, j, k, l
        logical error
        error=.false.
    ...
    subroutine split_segment (a, b, ns, xps, ids, outside)
        real(kind=dp), dimension ( 3 ), intent (in)  :: a, b     !Endpoints of line
        real(kind=dp), pointer :: xps(:,:) !segment endpoints
        ...
        real(kind=dp),              dimension ( 3 ) :: dab !b - a
        real(kind=dp), allocatable, dimension ( : ) :: t !parametric line intervals
        real(kind=dp) :: x !intersection point, stores x, y or z values
        integer      :: i, j, k
    ...
    real(kind=dp) function dsqrd ( a, b, R ) result ( d )
        real(kind=dp), dimension ( 3 ), intent (in) :: a, b !Endpoints
        real(kind=dp), dimension (:,:), intent (in) :: R    !Anisotropic rotation matrix
        real(kind=dp), dimension ( 3 )      :: ba   !Vector b - a
        ba = matmul ( R, b - a )
        d  = dot_product ( ba, ba )
    end function dsqrd
    !same function but for a vector b - a
    real(kind=dp) function dsqrd_vect ( ba, R ) result ( d )
        real(kind=dp), dimension ( 3 ), intent (in) :: ba !Vector b - a
        real(kind=dp), dimension (:,:), intent (in) :: R  !Anisotropic rotation matrix
    ...
    real(kind=dp) function dsqrd_iso ( a, b ) result ( d )
        real(kind=dp), dimension ( 3 ), intent (in) :: a, b
        real(kind=dp), dimension ( 3 ) :: ba
        ba = b - a
        d = dot_product( ba, ba )
    end function dsqrd_iso
    !same function for isotropic vector
    real(kind=dp) function dsqrd_iso_vect ( ba ) result ( d )
        real(kind=dp), dimension ( 3 ), intent (in) :: ba
        d = dot_product( ba, ba )
    end function dsqrd_iso_vect
    !same function for a line L
    real(kind=dp) function dsqrt_line ( n, L, outside ) result ( d )
        integer, intent (in) :: n !number of vertices including endpoints
        real(kind=dp), dimension (n,3), intent (in) :: L   !line
    ...
    subroutine fix_angles ( angles )
        real(kind=dp), dimension ( 3 ), intent (inout) :: angles
    ...
    subroutine get_rmatrix (angles, ratios, R)
        ...
        real(kind=dp), dimension ( 2 ), intent (in)    :: ratios
        real(kind=dp), dimension (3,3), intent (inout) :: R
        real(kind=dp) :: sina, sinb, sint, cosa, cosb, cost
...
!    real(kind=dp) function backtrack ( D, G, L0, n, verts, Fn ) result ( t )
!        real(kind=dp), parameter :: phi = 1.618033988749895D+00
!
!        real, parameter :: alpha = 0.01
!        real, parameter :: beta  = 0.9
!        integer, intent (in) :: n  !number of vertices
!        real(kind=dp), intent (in) :: L0 !initial length of line
!        real(kind=dp), dimension ( : ),   intent (in) :: D, G  !descent direction and gradient
!        real,         dimension ( n, 3), intent (in) :: verts !line vertices
!        real(kind=dp), intent (inout) :: Fn
!        !INTERNALS
!        integer :: i
!        logical :: outside
!        real(kind=dp) :: C0
...
!    end function backtrack
    ...
    real(kind=dp) function goldsection ( D, L0, n, Line, f ) result ( u )
        real(kind=dp), parameter :: phi  = 1.618033988749895D+00
        real(kind=dp), parameter :: beta = 0.9
        integer, intent (in) :: n  !number of vertices
        real(kind=dp), intent (in) :: L0 !initial length of line
        real(kind=dp), dimension ( : ),   intent (in) :: D  !descent direction
        real(kind=dp), dimension ( n, 3), intent (in) :: Line !line vertices
        real(kind=dp), intent (inout) :: f
        !INTERNALS
        real(kind=dp), dimension (n,3) :: tline
        real(kind=dp) :: fa, fb, fc, df
        real(kind=dp) :: ua, ub, uc
        integer :: i
